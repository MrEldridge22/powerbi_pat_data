...
def process_pat_m(excel_file, test_level, df):
    # Read the Excel file
    patM = pd.read_excel(excel_file, header=None)
    ...
    question_info = patM[patM.iloc[:, 0].isin(['Question difficulty', 'Strand', 'Percentage correct', 'Question number'])]
    ...
    # Add the test level to the melted DataFrame
    patM_melted['Test Level'] = test_level
    # Add the strand information to the melted DataFrame
    patM_melted = patM_melted.merge(question_info, left_on='QuestionNumber', right_index=True, how='left')
    # Reorder the columns
    patM_melted = patM_melted[['Unique ID', 'Completed', 'Score', 'Scale', 'Stanine', 'Percentile', 'Test Level', 'QuestionNumber', 'Response'] + list(question_info.columns)]

    df = pd.concat([df, patM_melted], ignore_index=True)
    return df
